...
class FedEdgeServer(FedEdgeServerInterface):
    ...
    def forward_propagation(self, client_ip, sharedData):
        ...
        fed_logger.info(
            Fore.GREEN + f"{str(client_ip)} offloading training started")
        ...
        while flag:
            if self.split_layers[config.CLIENTS_CONFIG.get(client_ip)][0] < model_utils.get_unit_model_len() - 1:
                ...
                if self.split_layers[config.CLIENTS_CONFIG[client_ip]][0] < \
                    ...
                    wall_time_comp_time += time.time() - wall_time_s_time
    ...
    def energy(self, client_ips):
        """
        gets comp energy,comm energy, training time, remaining energy and utilization of clients and send them to server
        :param client_ips:
        :return:
        """
        energy_tt_list = []
        ...
        for c in config.CLIENTS_CONFIG.keys():
            if client_ips.__contains__(c):
                ...
                if self.simnet:
                    energy_tt[c] = [ms[1], ms[2], ms[3], ms[4], ms[5], self.process_wall_time[c],
                                    self.total_computation_time_on_edge]
                    energy_tt_list.append(
                        [ms[1], ms[2], ms[3], ms[4], ms[5], self.process_wall_time[c],
                         self.total_computation_time_on_edge])
                else:
                    energy_tt[c] = [ms[1], ms[2], ms[3], ms[4], ms[5], self.process_wall_time[c],
                                    self.total_computation_time_on_edge]
