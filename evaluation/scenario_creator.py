...
def topology_prompt(ctx, param, value):
    if ctx.params.get("decentralized") and not ctx.params.get("d2d"):
        return click.prompt(
            "Topology type",
            type=click.Choice(["ring", "fully_connected"]),
            default="ring",
        )
    return None
...
def d2d_cluster_topology_prompt(cluster_number):
    return click.prompt(
        f"Topology for Cluster {cluster_number}",
        type=click.Choice(["ring", "star"]),
        default="ring",
    )
...
def select_clients_interactive(available_clients, edge_or_cluster_number):
    choices = [
        questionary.Choice(title=f"Client {c}", value=c) for c in available_clients
    ]
...
def select_clients(available_clients, edge_or_cluster_number):
    if is_interactive():
        try:
            ...
            return select_clients_non_interactive(
                available_clients, edge_or_cluster_number
            )
    else:
...
def create_d2d_cluster_neighbors(cluster_clients, topology, client_ports, server_port):
    ...
    if topology == "ring":
        for i, cid in enumerate(cluster_clients):
            ...
            if i > 0:
                neighbors.append(
                    f"client{cluster_clients[i - 1]},{client_ports[cluster_clients[i - 1]]}"
                )
...
@click.option(
    "--decentralized", is_flag=True, default=True, prompt="Is it decentralized?"
)
@click.option(
    "--offloading", is_flag=True, default=False, prompt="Is offloading enabled?"
)
@click.option(
    "--splitting-method",
    default="fake_decentralized_splitting",
...
@click.option(
    "--client-memory-limit", default="512M", prompt="Memory limit for clients"
)
...
@click.option(
    "--dataset", default="cifar10", prompt="Dataset name (e.g., cifar10, cifar100)"
)
def create_docker_compose(
    ...
    scenario_description = (
        f"{'Decentralized' if decentralized else 'Centralized'} "
    ...
    if d2d:
        ...
        for cluster_index in range(1, num_clusters + 1):
            ...
            if not cluster_clients:
                print_with_color(
                    f"Warning: Cluster {cluster_index} has no clients!", Fore.YELLOW
                )
                continue
            for cid in cluster_clients:
        ...
        for cluster_index, cluster_clients in clusters:
            ...
            cluster_neighbors = create_d2d_cluster_neighbors(
                cluster_clients, cluster_topology, client_ports, server_port
            )
        ...
        server_neighbors = [
            f"client{i},{client_ports[i]}" for i in sorted(client_ports)
        ]
        server = create_node(
            server_port,
            decentralized,
        ...
        for edge_index, client_list in edge_clients.items():
            ...
            for client_index in client_list:
                data["clients"][client_index - 1][
                    "neighbors"
                ] = f"edge{edge_index},{edge_node['port']}"
